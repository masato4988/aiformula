...
wheel_base_(get_parameter("wheelbase").as_double())
{
    ...
    first_path_subscriber_ = this->create_subscription<nav_msgs::msg::Path>("/first_gnss_path", 10, std::bind(&Follower::firstPathCallback, this, std::placeholders::_1));
    second_path_subscriber_ = this->create_subscription<nav_msgs::msg::Path>("/second_gnss_path", 10, std::bind(&Follower::secondPathCallback, this, std::placeholders::_1));
    image_sub_ = this->create_subscription<sensor_msgs::msg::Image>("/zed/zed_node/rgb/image_rect_color", 10, std::bind(&Follower::imageCallback, this, std::placeholders::_1));
...
void Follower::imageCallback(const sensor_msgs::msg::Image::SharedPtr img){
    ...
    if (!largest_contour.empty()) {
        ...
        cv::drawContours(frame, std::vector<std::vector<cv::Point>>{largest_contour}, -1, cv::Scalar(255, 0, 0), 2);
    }
